...
; (format t "The value is ~a ~%" (all-moves 0 0 data))
...
(defun look (Potetialist Data)
	(let* (  (cell (first Potetialist))  (row (first cell))  (col (second cell)) )
		(cond 	((null Potetialist) nil)
				((equal T (mymember (list 7 6) Potetialist))
					(setq path Potetialist ) 
		  		)
				; ((equal T (mymember (list row col) (rest Potetialist))) nil)
		  		(T (let* ( (subLevelPotentialist (all-moves row col Data)) )
		  			(dolist (firstElement subLevelPotentialist)
		  				( cond( (and (not (mymember firstElement Potetialist)) 
		  							 (setq path (look (cons firstElement Potetialist) Data)))
		  					(return-from look path) )
		  					(T nil)
		  				)
		  			)
		  		   )
...
; (format t "All moves: ~a~%" (all-moves 5 5 data) )

(format t "Done, movelist = ~A~%~%" (look (list (list 0 0)) Data) )

(setq newPath (reverse (look (list (list 0 0)) Data)))
(defun myprint (newPath Data)
	(let* ( (cellone (first newPath)) (celltwo (second newPath)))
			(cond ((not (equal (rest newPath) nil))
					(format t "Moving from ~A to ~A~%" cellone celltwo)
					(myprint (rest newPath) Data)
				  )
				  (T (format t "~%"))
			)
	)
)
(myprint (look newPath Data ) Data)
(exit)